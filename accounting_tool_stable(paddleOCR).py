'''
    ...
    PaddleOCR安装：https://github.com/PaddlePaddle/PaddleOCR/blob/release/2.7/doc/doc_ch/quickstart.md
'''
...
class books:
    ...
    def chooseFiles(self):
        ...
        self.ui.filesName.setEnabled(False)
        ...
        if image_path and os.path.exists(image_path):
            for file in os.listdir(image_path):
                ...
                try:
                    ...
                    result = self.OCR.ocr(img_path, cls=True) # img_num * detect_num * 2 (例：[[[1399.0, 34.0], [1663.0, 36.0], [1663.0, 69.0], [1398.0, 67.0]], ('2022/03/31 13:00', 0.9633936882019043)])
                    txts = [line[1][0] for line in result[0]]
                    # 获取有用数据:store_name sea_Foods
                    store_name = ""
                    got_store = False
                    sea_Foods = []
                    for txt in txts:
        ...
        if os.path.exists(wbName):
            ...
            for row in ws.rows:
                ...
                for i in range(len(line) - 2):
                    tableWeight.setItem(insertPosition, i,
                                        QTableWidgetItem(line[i] if isinstance(line[i], str) else str(line[i])))
                for i in [1, 2]:
